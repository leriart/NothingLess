...
ENDCONF
	...
	if [ "$MODE" = "lua" ]; then
		# Write valid Lua config header — sync-hyprland.py injects hl.config()
		# and hl.bind() blocks below. Must NOT use return [[...]] because the
		# injected blocks are pure hl.* API calls, not conf syntax strings.
		#
		# exec-once uses hl.config() since hl.exec() does not exist in the
		# Hyprland Lua API. Multiple hl.config() calls merge at runtime.
		{
			...
			printf "hl.config({\n"
			printf "    exec = {\n"
			printf "        once = { \"sh -c '[ -f /tmp/.nl_booted ] || { touch /tmp/.nl_booted && nothingless; }'\" }\n"
			printf "    }\n"
			printf "})\n"
			printf "\n"
		...
		# Remove stale source line from .conf so it doesn't load both
		...
		# Remove stale source line from .lua so it doesn't load both
